        ...
        public override void WriteUser(ReadOnlySpan<byte> data, ulong addr)
        {
            ...
            unsafe
            {
                fixed (byte* dataPtr = data)
                {
                    ...
                    if (bytesWritten != data.Length && (!((Int32)data.Length ==1 && bytesWritten ==4) || Marshal.GetLastWin32Error() != 0)) // Byte written should be either as requested, or in the case of a single byte, padded to 4 bytes TODO: or all writes < 4 bytes ??
